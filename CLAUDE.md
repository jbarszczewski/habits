...
- Simple MVVM: `ui/` (screens + ViewModels), `data/` (Room + repository), `widget/`, `timer/`
- No dependency-injection framework; use a hand-written `AppContainer` in the Application class
...
- Timed task status: `actual_minutes >= target_minutes` -> DONE, `0 < actual < target` -> PARTIAL.
...
- Per task: current streak, longest streak, done / partial / skipped / missed
  counts, completion rate (strict: DONE only; weighted: actual/target capped at 1)
...
- Any write from the widget goes through the same repository as the app,
  then calls `updateAll()` on the widget.
...
2. Today screen: list today's tasks with checkbox / progress; mark done. Seed one
   test task in debug builds.
...
- Run `./gradlew assembleDebug` and `./gradlew testDebugUnitTest` before saying a
  step is complete.
